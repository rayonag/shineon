...
let uploadFile;
{
    ...
    function getFile() {
        ...
        if (options.image || options.readMethod == 'readAsDataURL') reader.readAsDataURL(this.files[0]);
...
Side.prototype.twist = function (ca, cb, tgx, tgy) {
    ...
    this.points = [
        ...
        new Point(pc.x - (2 / 12) * dxh * 0.52 * tgx, pc.y - (2 / 12) * dyh * 0.52 * tgy),
        pc,
        new Point(pc.x + (2 / 12) * dxh * 0.52 * tgx, pc.y + (2 / 12) * dyh * 0.52 * tgy),
...
Piece.prototype.createDivPiece = function (puzzle, scale, offsx, offsy) {
    ...
    ctx.drawImage(puzzle.image, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight);
    // shadow effect
    ...
    this.moveTo(new Point((this.kx - 1) * puzzle.dx + Puzzle.MARGIN1, (this.ky - 1) * puzzle.dy + Puzzle.MARGIN1));

    this.theDiv.appendChild(this.theCanv);
    return this.theDiv;
}; // createDivPiece
Piece.prototype.createBoardDivPiece = function (puzzle, scale, offsx, offsy) {
    let ctx, shiftx, shifty;
    const div2 = document.createElement('div');
    div2.style.width = 3 * puzzle.dx + 'px';
    div2.style.height = 3 * puzzle.dy + 'px';
    div2.style.position = 'absolute';

    const canv2 = document.createElement('canvas');
    canv2.width = 3 * puzzle.dx;
    canv2.height = 3 * puzzle.dy;
    //-----------------------------------------------
    ...
    this.drawPath(ctx2, shiftx, shifty);
    ctx2.clip();
    //-----------------------------------------------
    ...
    //-----------------------------
    this.drawPath(ctx2, shiftx - shift, shifty - shift);
    ...
    /* place of the piece at the beginning of the game */
    const onePoint = new Point((this.kx - 1) * puzzle.dx + Puzzle.MARGIN1, (this.ky - 1) * puzzle.dy + Puzzle.MARGIN1);
    div2.style.top = onePoint.y + 'px';
    div2.style.left = onePoint.x + 'px';
    //-----------------------------
    div2.appendChild(canv2);
    document.getElementById('mainBoard').appendChild(div2);
    //-----------------------------
    return div2;
}; // createBoardDivPiece
// -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -   -
...
*/
Piece.prototype.moveTo = function (onePoint, div2 = null) {
...
PolyPiece.prototype.ifNear = function (otherPoly) {
    ...
    for (let kp = 0; kp < this.pieces.length; ++kp) {
        ...
        for (let kn = 0; kn < otherPoly.pieces.length; ++kn) {
            ...
            // p2 above p1 ?
            if (p1.kx == p2.kx && p1.ky == p2.ky + 1 && this.puzzle.near(p1, p2, 0, -1)) return true;
            // p2 below p1 ?
            if (p1.kx == p2.kx && p1.ky == p2.ky - 1 && this.puzzle.near(p1, p2, 0, 1)) return true;
            // p2 left of p1 ?
            if (p1.kx - 1 == p2.kx && p1.ky == p2.ky && this.puzzle.near(p1, p2, -1, 0)) return true;
            // p2 right of p1 ?
            if (p1.kx + 1 == p2.kx && p1.ky == p2.ky && this.puzzle.near(p1, p2, +1, 0)) return true;
        } // for kn
    } // for kp
...
*/
    ...
    this.canvMobile.getContext('2d').drawImage(this.image, 0, 0, wi, he, Puzzle.MARGIN1, Puzzle.MARGIN1, this.width, this.height);
...
Puzzle.prototype.loadImage = function () {
    ...
    return function () {
        puz.menu.collapse();
        uploadFile(() => {}, { accept: 'image/*', readMethod: 'readAsDataURL', image: puz.image });
    };
};
...
Puzzle.prototype.next = function () {
    ...
    if (this.image.width / this.image.height < (this.reqWidth - 2 * Puzzle.MARGIN1) / (this.reqHeight - 2 * Puzzle.MARGIN1)) {
    ...
    this.divGame.style.width = this.divBoard.style.width = this.canvMobile.width + 'px';

    this.canvMobile.height = this.reqHeight;
    this.divGame.style.height = this.divBoard.style.height = this.canvMobile.height + 'px';
...
*/
    ...
    for (ky = 0; ky < ny; ++ky) {
        ...
        for (kx = 0; kx < nx; ++kx) {
            ...
            if (kx < nx - 1) {
                ...
                else np.rs.twist(corners[ky][kx + 2], corners[ky + 1][kx + 2], 0.5, 1);
            }
            // left side
            if (kx == 0) {
            ...
            if (ky < ny - 1) {
                ...
                else np.bs.twist(corners[ky + 2][kx], corners[ky + 2][kx + 1], 1, 0.5);
            }
        } // for kx
    } // for ky
    ...
    boardCanvas.id = 'mainBoard';
    boardCanvas.width = this.width; // Set to the width of the board
    boardCanvas.height = this.height; // Set to the height of the board
    this.divBoard.appendChild(boardCanvas);
    // --------------------------------------------------------------
...
Puzzle.prototype.associateImage = function () {
    ...
    this.pieces2 = [...this.pieces]; // copy of this.pieces
    this.boardPolyPieces = [];

    for (ky = 0; ky < this.ny; ky++) {
        for (kx = 0; kx < this.nx; kx++) {
            this.pieces2[ky][kx].createBoardDivPiece(this, scale, offsx, offsy);
            this.boardPolyPieces.push(new PolyPiece(this.pieces2[ky][kx], this));
            this.pieces[ky][kx].createDivPiece(this, scale, offsx, offsy);
            this.polyPieces.push(new PolyPiece(this.pieces[ky][kx], this));
        } // for kx
    } // for ky
    console.log('polyPieces', this.polyPieces);

    // random zindex for initial pieces
    arrayShuffle(this.polyPieces);
    this.evaluateZIndex();

    this.boardPolyPieces.forEach((polyPiece2) => {
        const where = polyPiece2.where();
        console.log('where', where);
        console.log('polyPiece2', polyPiece2);
    });
    ...
    for (kp = 0; kp < this.polyPieces.length; kp++) {
        for (kn = 0; kn < this.polyPieces[kp].pieces.length; kn++) {
            ...
            switch (this.freeSpace) {
                case 0:
                    pc.pTarget = new Point(this.reqWidth - (2.25 + Math.random() / 4) * this.dx, Math.random() * (this.height - this.dy) - this.dy);
                    break;
                case 1:
                    pc.pTarget = new Point(Math.random() * (this.width - this.dx) - this.dx, this.reqHeight - (2.25 + Math.random() / 4) * this.dy);
            } // switch
        } // for kn
    } // for kp
    window.setTimeout(
...
Puzzle.prototype.mouseUpGame = function (event) {
    ...
    do {
        ...
        console.log('this.polyPieces', this.polyPieces);
        for (k = 0; k < this.polyPieces.length; k++) {
            if (k === idp) continue; // don't check neighborhood with itself !
            //------------------------------------
            const mainBoard = document.getElementById('mainBoard');
            console.log('nrew', polyP.ifNear(this.boardPolyPieces[k]));
            console.log('this.this.polyPieces[k][k]', this.polyPieces[k]);
            //------------------------------------
    ...
    let dist = Math.sqrt(
        (this.anim.xorg - this.anim.xfin) * (this.anim.xorg - this.anim.xfin) + (this.anim.yorg - this.anim.yfin) * (this.anim.yorg - this.anim.yfin)
    );
...
function lookForLoops(tbCases) {
    ...
    for (k = 0; k < tbCases.length; k++) {
        for (kEdge = 0; kEdge < 4; kEdge++) {
            if (!edgeIsCommon(tbCases[k].kx, tbCases[k].ky, kEdge)) tbEdges.push({ kx: tbCases[k].kx, ky: tbCases[k].ky, edge: kEdge, kp: k });
        } // for kEdge
    } // for k
